...
export default async function CristianosPreguntasPage() {
  ...
  return (
    <div>
      ...
      <p className="mb-4 text-sm text-slate-500 dark:text-slate-400">{questions.length} pregunta(s)</p>

      <div className="space-y-4">
        {questions.map((q) => (
          <div
            key={q.id}
            className={`rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900 p-4 ${
              q.active ? "" : "opacity-50"
            }`}
          >
            <div className="mb-3 flex flex-wrap items-start justify-between gap-2">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">{q.active ? "Activa" : "Inactiva"}</p>
              </div>
            ...
            <ol className="grid grid-cols-1 gap-1 text-sm sm:grid-cols-2">
              {q.answers.map((a) => (
                <li
                  key={a.id}
                  className="flex items-center justify-between rounded bg-white px-3 py-1.5 dark:bg-slate-800"
                >
                  <span>
                    <span className="mr-2 font-mono text-amber-600 dark:text-amber-400">{a.rank}.</span>
                    {a.text}
                  </span>
                  <span className="font-mono text-slate-500 dark:text-slate-400">{a.points}</span>
                </li>
              ))}
            </ol>
          </div>
