...
export default async function SesionCristianosAdminPage({
  ...
  return (
    <div>
      ...
      <p className="mb-6 text-sm text-slate-400">Estado: {session.status}</p>

      <div className="mb-6 grid grid-cols-2 gap-4 sm:w-96">
        <div className="rounded-lg border border-slate-800 bg-slate-900 p-4">
          <p className="text-sm text-slate-400">{session.teamAName}</p>
          <p className="text-2xl font-bold">{session.teamAScore.toLocaleString("es")}</p>
        </div>
        <div className="rounded-lg border border-slate-800 bg-slate-900 p-4">
          <p className="text-sm text-slate-400">{session.teamBName}</p>
          <p className="text-2xl font-bold">{session.teamBScore.toLocaleString("es")}</p>
        </div>
      </div>

      <ul className="mb-6 divide-y divide-slate-800 rounded-lg border border-slate-800">
