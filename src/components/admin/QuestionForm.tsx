...
export function QuestionForm({
  ...
  return (
    <form action={formAction} className="max-w-2xl space-y-5">
      <div>
        <label className="mb-1 block text-sm text-slate-300">Pregunta</label>
        <textarea
          name="text"
          required
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        {(["A", "B", "C", "D"] as const).map((key) => (
          <div key={key}>
            <label className="mb-1 block text-sm text-slate-300">Opción {key}</label>
            <input
              name={`option${key}`}
              required
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <div>
          <label className="mb-1 block text-sm text-slate-300">Opción correcta</label>
          <select
            name="correctOption"
        ...
        <div>
          <label className="mb-1 block text-sm text-slate-300">Dificultad (1-15)</label>
          <input
            name="difficulty"
            type="number"
            min={1}
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div>
          <label className="mb-1 block text-sm text-slate-300">Versión bíblica</label>
          <input
            name="bibleVersion"
